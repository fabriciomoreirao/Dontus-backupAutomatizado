...
export class BackupRepository {
  ...
  async backupAtendimentosRealizadosOrto(
    ...
    const query = `
      SELECT 
        ...
        SERVICOS.ID AS ID_PROCEDIMENTO,
        SERVICOS.DESCRICAO AS PROCEDIMENTO,
        CONTAS_RECEBER.ID AS ID_CONTA_RECEBER,
        CONTAS_RECEBER.VALOR_TOTAL, 
        PACIENTES.ID AS ID_PACIENTE,
        PACIENTES.NOME AS PACIENTE, 
        FUNCIONARIOS.ID AS ID_PROFISSIONAL,
        ORTO.ID AS ID_ORTO,
        ORCAMENTOSITENS.ID_ORCAMENTO AS ID_ORCAMENTO,
        FUNCIONARIOS.NOME AS PROFISSIONAL, 
      ...
      LEFT JOIN ORCAMENTOSITENS ON ATENDIMENTOS.ID_ORCAMENTOS_ITENS = ORCAMENTOSITENS.ID
      JOIN PACIENTES ON ATENDIMENTOS.ID_PACIENTE = PACIENTES.ID
      LEFT JOIN SERVICOS ON ATENDIMENTOS.ID_SERVICO = SERVICOS.ID
      JOIN CONTAS_RECEBER ON ATENDIMENTOS.ID_CONTAS_RECEBER = CONTAS_RECEBER.ID
      JOIN ORTO ON CONTAS_RECEBER.ID_ORTO = ORTO.ID
      WHERE PACIENTES.ID_CLINICA = ${clinicaId}
    `;
  ...
  async backupContasRecebidas(
    ...
    const query = `
      ...
      LEFT JOIN ORCAMENTOS ON CONTAS_RECEBER.ID_ORCAMENTO = ORCAMENTOS.ID
      LEFT JOIN SERVICOS ON CONTAS_RECEBER.ID_SERVICO = SERVICOS.ID
  ...
  async backupProcedimentosOrcamento(
    ...
    const query = `
      SELECT 
        ...
        TABELA_SERVICO.ID AS ID_TABELA,
        TABELA_SERVICO.DESCRICAO AS TABELA, 
        ESPECIALIDADES.ID AS ID_ESPECIALIDADE,
        ESPECIALIDADES.DESCRICAO AS ESPECIALIDADE, 
        SERVICOS.ID AS ID_PROCEDIMENTO,
        SERVICOS.DESCRICAO AS PROCEDIMENTO, 
        CASE WHEN ATENDIMENTOS_ITENS.ID IS NOT NULL THEN 1 ELSE 0 END AS EXECUTADO,
      ...
      LEFT JOIN ATENDIMENTOS_ITENS ON ATENDIMENTOS.ID = ATENDIMENTOS_ITENS.ID_ATENDIMENTO
  ...
  async backupProcedimentosContaAvulsaEOrto(
    ...
    const query = `
      SELECT 
        ATENDIMENTOS.ID AS ID_ATENDIMENTO,
        SERVICOS.ID AS ID_PROCEDIMENTO,
        TABELA_SERVICO.ID AS ID_TABELA,
        ESPECIALIDADES.ID AS ID_ESPECIALIDADE,
        PACIENTES.ID AS ID_PACIENTE,
        CONTAS_RECEBER.ID AS ID_CONTA_RECEBER,
        CONTAS_RECEBER.ID_ORTO AS ID_ORTO,
        PACIENTES.NOME AS PACIENTES, 
        CONTAS_RECEBER.VALOR_TOTAL, 
        TABELA_SERVICO.DESCRICAO AS TABELA, 
        ESPECIALIDADES.DESCRICAO AS ESPECIALIDADE,
  ...
  async backupContasPagas(
    ...
    const query = `
      SELECT 
        CONTASPAGAR.ID AS ID_CONTAS_PAGAR, 
        FUNCIONARIOS.ID AS ID_PROFISSIONAL,
  ...
  async backupContasAPagar(
    ...
    const query = `
      SELECT 
        ...
        FUNCIONARIOS.NOME AS PROFISSIONAL, 
        FORNECEDORES.RAZAOSOCIAL AS FORNECEDOR, 
        CENTROCUSTO.ID AS ID_CENTRO_CUSTO,
        CENTROCUSTO.DESCRICAO, 
        CONTASPAGAR.DATA_VENCIMENTO,
        CASE WHEN CONTASPAGAR.ID_STATUS = 1 THEN 1 ELSE 0 END AS PAGO,
        CASE WHEN CONTASPAGAR.ID_PAGAMENTO_FUNCIONARIO IS NOT NULL THEN 1 ELSE 0 END AS PAGAMENTO_DIARIO,
        CONTASPAGAR.VALOR
      FROM CONTASPAGAR
      LEFT JOIN CENTROCUSTO ON CONTASPAGAR.ID_CENTRO_CUSTO = CENTROCUSTO.ID
      LEFT JOIN FORNECEDORES ON CONTASPAGAR.ID_FORNECEDOR = FORNECEDORES.ID
